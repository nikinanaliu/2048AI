...
public class New2048 {
    ...
    static float score_toplevel_move(long board, int move) {
        ...
        state.setDepth_limit(Max(3, count_distinct_tiles(board) - 2
        ));
       ...
       // System.out.printf("Move %ld: result %lf: eval'd %ld moves (%ld cache hits, %ld cache size) in %lf seconds (maxdepth=%ld)\n", move, res, state.getMoves_evaled(), state.getCachehits(),  state.getTrans_table().size(), elapsed, state.getMaxdepth());
